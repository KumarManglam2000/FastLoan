...
namespace UserService.Controllers
{
    ...
    {
        internal readonly IMLService _userService;

        private readonly string bootstrapServers = "localhost:9092";
        private readonly string topic = "test6";
        public MLController(IMLService userService)
        {
        ...
        [HttpPost]
        [Route("")]
        public IActionResult Registeruser(MLRegistration user)
        {
            try
            {
                MLRegistration isAddusersuccess = _userService.RegisterUser(user);
        ...
        [HttpPut]
        [Route("{Email}")]
        public IActionResult UpdateUser(String Email, MLRegistration user)
        {
            try
            {
                MLRegistration updateStatus = _userService.UpdateUser(Email, user);
        ...
        public ActionResult FindUser(String Email)
        {
            try
            {
                MLRegistration user = _userService.GetUserByEmail(Email);
                return Ok(user);
            }
            catch (UserNotFoundException unfx)
            {
